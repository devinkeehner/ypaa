...
function Editable({ as: Tag = "span", props, field, children, className }: { as?: ElementType; props: Base; field: string; children: ReactNode; className?: string }) {
  ...
  const isRichValue = Boolean(children && typeof children === "object" && !Array.isArray(children) && "root" in (children as object));
  if (isRichValue) return <div className={`${className || ""} ${styles.richInline}`} data-puck-text-field={field} onFocus={focus} onPointerDown={focus} style={styleFor(props, field)}><PayloadRichText data={normalizeLexicalValue(children)} /></div>;
...
function RichCopy({ value, as: Tag = "p", className }: { value: unknown; as?: ElementType; className?: string }) {
  const isRichValue = Boolean(value && typeof value === "object" && !Array.isArray(value) && "root" in (value as object));
  return isRichValue ? <div className={`${className || ""} ${styles.richInline}`}><PayloadRichText data={normalizeLexicalValue(value)} /></div> : <Tag className={className}>{typeof value === "string" ? value : ""}</Tag>;
}
...
function BioSectionRender({ props }: { props: CampaignAltProps }) {
  return <section className={`${styles.campaignAlt} ${styles.bioSectionAlt}`} data-presentation={props.presentation} data-variant={props.variant} id={props.id}><div className={styles.shell}><CampaignImage className={styles.campaignAltProfileMedia} media={props.media} /><div className={styles.bioSectionCopy}><CampaignSectionHeader props={props} />{props.body && props.intro ? <Editable as="p" className={styles.campaignAltLongCopy} field="body" props={props}>{props.body}</Editable> : null}{props.items.length ? <div className={styles.bioHighlights}>{props.items.map((item, index) => <div key={`${item.text}-${index}`}><CampaignIcon name={item.icon} /><RichCopy value={item.text || item.heading} /></div>)}</div> : null}</div></div></section>;
}
...
function PalmCardPointsRender({ props }: { props: CampaignAltProps }) {
  return <section className={`${styles.campaignAlt} ${styles.palmPointsAlt}`} data-presentation={props.presentation} data-variant={props.variant} id={props.id}><div className={styles.shell}><CampaignSectionHeader centered props={props} /><div className={styles.palmPointsGrid}>{props.items.map((item, index) => <article key={`${item.text}-${index}`}><CampaignIcon name={item.icon} /><RichCopy value={item.text || item.heading} /></article>)}</div></div></section>;
}
...
function CampaignAltTabs({ props }: { props: CampaignAltProps }) {
  const [active, setActive] = useState(0);
  return <div className={styles.campaignAltTabs} data-variant={props.variant}><div role="tablist">{props.tabs.map((tab, index) => <button aria-selected={active === index} key={`${tab.label}-${index}`} onClick={() => setActive(index)} role="tab" type="button"><RichCopy as="span" value={tab.label || tab.heading || `Tab ${index + 1}`} /></button>)}</div>{props.tabs.map((tab, index) => <section hidden={!props.puck?.isEditing && active !== index} key={`${tab.heading}-${index}`}><RichCopy as="h3" value={tab.heading} />{tab.text ? <RichCopy value={tab.text} /> : null}<SlotContent content={tab.blocks} label={tab.label || `tab ${index + 1}`} fallback={renderZone(props, `tabs.${index}.blocks`, `Drop elements in ${tab.label || `tab ${index + 1}`}`)} /></section>)}</div>;
}
...
export const puckConfig: Config<Components> = {
  ...
  components: {
    ...
    ProgramSchedule: {
      ...
      render: (props) => <section id={props.id}><ProgramExplorer embedded heading={typeof props.heading === "string" ? props.heading : ""} introduction={typeof props.introduction === "string" ? props.introduction : ""} /></section>,
    },
    ButtonRow: {
      label: "Button row",
