...
const bottomElementsField: Field<boolean> = {
  type: "custom",
  label: "Bottom element area",
  render: ({ value, onChange, readOnly }) => <label style={{ alignItems: "center", display: "flex", gap: 9 }}><input checked={value !== false} disabled={readOnly} onChange={(event) => onChange(event.currentTarget.checked)} type="checkbox" /><span>Show an element drop area near the bottom</span></label>,
};
...
export const puckConfig: Config<Components> = {
  ...
  components: {
    ...
    RichText: {
      label: "Rich text",
      defaultProps: { content: "Write rich text here.", fontSize: "1rem", color: "var(--tenant-dark-text, #171614)", fontWeight: "400", alignment: "left" },
    ...
    Text: { label: "Text", defaultProps: { text: "Add text", fontSize: "1rem", color: "var(--tenant-dark-text, #171614)", alignment: "left" }, fields: { text: area("Text"), fontSize: plainText("Font size"), color: themeColorField("Color"), alignment: { type: "select", label: "Alignment", options: [{ label: "Left", value: "left" }, { label: "Center", value: "center" }, { label: "Right", value: "right" }] } }, render: (props) => <section className={styles.freeText} id={props.id} style={{ color: props.color, fontSize: props.fontSize, textAlign: props.alignment }}><Editable field="text" props={props}>{props.text}</Editable></section> },
