...
function CampaignBridge({ type, props }: { type: CampaignAltType; props: CampaignAltProps }) {
  ...
  return <div className={styles.campaignAltBottomElements}>
    <SlotContent content={props.afterContent} instruction="Section bottom · drag elements here" label={`${definition.label} bottom elements`} minEmptyHeight={76} purpose="bottom" tone={campaignBridgeTone(definition, props)} fallback={null} />
  </div>;
}
...
function campaignAltFields(definition: CampaignAltDefinition) {
  ...
  const bridge = AFTER_CONTENT_BLOCK_TYPES.has(definition.type) ? { afterContent: { type: "slot" as const, label: "Section bottom elements", allow: nestedElementTypes } } : {};
