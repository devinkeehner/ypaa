...
const bottomElementsField: Field<boolean> = {
  type: "custom",
  label: "Extra element drop area",
  render: ({ value, onChange, readOnly }) => {
    const enabled = value !== false;
    return <label className={styles.bottomElementsField} data-enabled={enabled}>
      <input checked={enabled} disabled={readOnly} onChange={(event) => onChange(event.currentTarget.checked)} type="checkbox" />
      <span><strong>{enabled ? "Drop area is on" : "Drop area is off"}</strong><small>{enabled ? "Open Elements and drag into the labeled area in the preview. Its position follows the selected section variant." : "Turn this on to add an element-only area after the section's main text."}</small></span>
    </label>;
  },
};
...
function CampaignBridge({ type, props }: { type: CampaignAltType; props: CampaignAltProps }) {
  ...
  if (!definition || !AFTER_CONTENT_BLOCK_TYPES.has(type) || props.enableAfterContent === false) return null;
  return <SlotContent content={props.afterContent} instruction="Extra elements · drag from the Elements tab" label={`${definition.label} extra elements`} minEmptyHeight={76} purpose="bottom" tone={campaignBridgeTone(definition, props)} fallback={null} />;
}
...
function campaignAltFields(definition: CampaignAltDefinition) {
  ...
  const bridge = AFTER_CONTENT_BLOCK_TYPES.has(definition.type) ? { enableAfterContent: bottomElementsField, afterContent: { type: "slot" as const, label: "Bottom elements", allow: nestedElementTypes } } : {};
