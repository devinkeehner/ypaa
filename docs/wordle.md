...
Before play, a reading screen opens on every fresh visit (including testing refreshes). In Daily word puzzles, enter Before-play reading title, Before-play passage, and Reading attribution. Only these public reading fields reach the client; the answer stays on the server. Blank passages display a link to the official AA Daily Reflections instead. Use original or authorized passages; the app does not import the copyrighted daily reader.

The share renderer uses public/images/word-puzzle-share-base.png, generated with the built-in image tool. Final prompt: clean dark-plum 1200:630 card, NECYPAA XXXVI and editorial serif Escaping the / italic Mad Realm. at left, existing hotel-character reference below, Hartford and event dates at bottom; right half empty for programmatic date, score, and tiles. The crossed-out day / handwritten word title and sharing footer are then baked into the PNG by scripts/bake-puzzle-card-title.cjs. Only date, score, and tiles render dynamically. The earlier ornate draft is not used.
