import { E as init_react_import, T as __spreadValues } from "./config-Z15zxwKW.js";
import { A as Plugin, C as mergeAttributes, D as textblockTypeInputRule, E as renderNestedMarkdownContent, I as Fragment, M as Selection, N as TextSelection, O as wrappingInputRule, S as markPasteRule, T as parseIndentedBlocks, _ as isAtEndOfNode, a as Node3, b as isNodeSelection, c as combineTransactionSteps, d as getChangedRanges, f as getMarksBetween, h as getRenderedAttributes, i as Mark, j as PluginKey, k as NodeSelection, l as findChildrenInRange, m as getNodeType, n as Extension, o as PasteRule, p as getNodeAtPosition, r as InputRule, s as canInsertNode, u as getAttributes, v as isAtStartOfNode, w as nodeInputRule, x as markInputRule, y as isNodeActive } from "./dist-DT7nY1jx.js";
...
//#endregion
//#region node_modules/@puckeditor/core/dist/chunk-2CNEFIQP.mjs
init_react_import();
var defaultPuckRichTextOptions = { textAlign: { types: ["heading", "paragraph"] } };
var PuckRichText = Extension.create({
	name: "puckRichText",
	addExtensions() {
		const extensions = [];
		const options = __spreadValues(__spreadValues({}, this.options), defaultPuckRichTextOptions);
		if (options.bold !== false) extensions.push(Bold.configure(options.bold));
		if (options.blockquote !== false) extensions.push(Blockquote.configure(options.blockquote));
		if (options.code !== false) extensions.push(Code.configure(options.code));
		if (options.codeBlock !== false) extensions.push(CodeBlock.configure(options.codeBlock));
		if (options.document !== false) extensions.push(Document.configure(options.document));
		if (options.hardBreak !== false) extensions.push(HardBreak.configure(options.hardBreak));
		if (options.heading !== false) extensions.push(Heading.configure(options.heading));
		if (options.horizontalRule !== false) extensions.push(HorizontalRule.configure(options.horizontalRule));
		if (options.italic !== false) extensions.push(Italic.configure(options.italic));
		if (options.listItem !== false) {
			extensions.push(ListItem.configure(options.listItem));
			if (options.bulletList !== false) extensions.push(BulletList.configure(options.bulletList));
			if (options.orderedList !== false) extensions.push(OrderedList.configure(options.orderedList));
		}
		if (options.listKeymap !== false) extensions.push(ListKeymap.configure(options == null ? void 0 : options.listKeymap));
		if (options.link !== false) extensions.push(Link.configure(options == null ? void 0 : options.link));
		if (options.paragraph !== false) extensions.push(Paragraph.configure(options.paragraph));
		if (options.strike !== false) extensions.push(Strike.configure(options.strike));
		if (options.text !== false) extensions.push(Text.configure(options.text));
		if (options.textAlign !== false) extensions.push(index_default.configure(options.textAlign));
		if (options.underline !== false) extensions.push(Underline.configure(options == null ? void 0 : options.underline));
		return extensions;
	}
});
//#endregion
export { PuckRichText as t };
