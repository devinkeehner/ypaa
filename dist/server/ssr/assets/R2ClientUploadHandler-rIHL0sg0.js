...
import { Qt as formatAdminURL, bn as se, ft as md, ht as ot } from "./client-CJQLBaQM.js";
import path from "path";
//#region node_modules/http-status/dist/chunk-CUNVWAK5.js
var e = {
	"1xx": "Informational",
...
//#endregion
//#region node_modules/payload/dist/errors/APIError.js
...
*/ var APIError = class extends ExtendableError {
	/**
	...
	*/ constructor(message, status = t.INTERNAL_SERVER_ERROR, data = null, isPublic) {
		super(message, status, data, typeof isPublic === "boolean" ? isPublic : status !== t.INTERNAL_SERVER_ERROR);
	}
};
//#endregion
//#region node_modules/payload/dist/utilities/sanitizeFilename.js
/**
...
*/ function sanitizeFilename(filename) {
	...
	if (!sanitized) throw new APIError("Invalid filename", 400);
	return sanitized;
}
//#endregion
//#region node_modules/@payloadcms/plugin-cloud-storage/dist/client/createClientUploadHandler.js
var import_jsx_runtime = require_jsx_runtime();
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var createClientUploadHandler = ({ handler }) => {
	return function ClientUploadHandler({ children, collectionSlug, enabled, extra, prefix, serverHandlerPath }) {
		const { setUploadHandler } = md();
		const { config: { routes: { api: apiRoute }, serverURL } } = se();
		const initializeHandler = ot(() => {
			if (enabled) setUploadHandler({
				collectionSlug,
...
//#endregion
//#region node_modules/@payloadcms/plugin-cloud-storage/dist/utilities/sanitizePrefix.js
/**
...
*/ function sanitizePrefix(prefix) {
	...
	if (/%[0-9a-f]{2}/i.test(decodedPrefix)) return "";
	return decodedPrefix.replace(/\\/g, "/").split("/").filter((segment) => segment !== ".." && segment !== ".").join("/").replace(/^\/+/, "").replace(/[\x00-\x1f\x80-\x9f]/g, "");
}
//#endregion
//#region node_modules/@payloadcms/plugin-cloud-storage/dist/utilities/getFileKey.js
/**
...
*/ function getFileKey({ collectionPrefix, docPrefix, filename, useCompositePrefixes = false }) {
	const safeCollectionPrefix = sanitizePrefix(collectionPrefix || "");
	const safeDocPrefix = sanitizePrefix(docPrefix || "");
	const safeFilename = sanitizeFilename(filename);
	return {
		fileKey: useCompositePrefixes ? path.posix.join(safeCollectionPrefix, safeDocPrefix, safeFilename) : path.posix.join(safeDocPrefix || safeCollectionPrefix, safeFilename),
...
//#endregion
//#region node_modules/@payloadcms/storage-r2/dist/client/R2ClientUploadHandler.js
var R2ClientUploadHandler = createClientUploadHandler({ handler: async ({ apiRoute, collectionSlug, docPrefix, extra: { chunkSize = 5 * 1024 * 1024 }, file, prefix, serverHandlerPath, serverURL, updateFilename }) => {
	...
	if (sanitizedFilename && sanitizedFilename !== file.name) updateFilename(sanitizedFilename);
