...
const initialOrder: RegistrationOrder = {
  ...
  scholarship: { enabled: false, kind: "general", amountCents: REGISTRATION_PRICE_CENTS, recipientName: "", recipientEmail: "", recipientState: "", recipientHomegroupCommittee: "", recipientAccommodations: "", recipientInterpretationNeeded: false, recipientMobilityAccessibility: false, recipientWillingToServe: false, attribution: "" },
};
      ...
      <main className="registration-page" id="registration-main">
        ...
        <div className="registration-shell registration-layout">
          <div className="registration-form-stack">
            ...
            <section className="registration-card"><div className="registration-card-heading"><div><h2>Registration scholarship</h2><p>Reserve one for a person or contribute to the general door fund.</p></div><label className="registration-switch"><input checked={order.scholarship.enabled} onChange={(e) => updateScholarship("enabled", e.target.checked)} type="checkbox" /><span>{order.scholarship.enabled ? "Included" : "Not included"}</span></label></div>{order.scholarship.enabled ? <div className="registration-grid"><label><span>Scholarship type</span><select onChange={(e) => updateScholarship("kind", e.target.value)} value={order.scholarship.kind}><option value="general">General scholarship fund</option><option value="specific">Specific person</option></select></label>{order.scholarship.kind === "general" ? <label><span>Contribution amount *</span><input min="40" onChange={(e) => updateScholarship("amountCents", Math.max(4000, Math.round(Number(e.target.value || 40) * 100)))} step="1" type="number" value={order.scholarship.amountCents / 100} /></label> : <><label><span>Recipient name *</span><input onChange={(e) => updateScholarship("recipientName", e.target.value)} value={order.scholarship.recipientName} /></label><label><span>Recipient email *</span><input onChange={(e) => updateScholarship("recipientEmail", e.target.value)} type="email" value={order.scholarship.recipientEmail} /></label><label><span>Recipient state *</span><input onChange={(e) => updateScholarship("recipientState", e.target.value)} value={order.scholarship.recipientState} /></label><label><span>Homegroup / committee</span><input onChange={(e) => updateScholarship("recipientHomegroupCommittee", e.target.value)} value={order.scholarship.recipientHomegroupCommittee} /></label><label className="registration-full"><span>Recipient accommodation needs</span><textarea onChange={(e) => updateScholarship("recipientAccommodations", e.target.value)} value={order.scholarship.recipientAccommodations} /></label><div className="registration-full registration-checks"><label><input checked={order.scholarship.recipientInterpretationNeeded} onChange={(e) => updateScholarship("recipientInterpretationNeeded", e.target.checked)} type="checkbox" /> Interpretation needed</label><label><input checked={order.scholarship.recipientMobilityAccessibility} onChange={(e) => updateScholarship("recipientMobilityAccessibility", e.target.checked)} type="checkbox" /> Mobility accessibility needed</label><label><input checked={order.scholarship.recipientWillingToServe} onChange={(e) => updateScholarship("recipientWillingToServe", e.target.checked)} type="checkbox" /> Willing to be of service</label></div><p className="registration-full">The recipient will be added to the attendee roster. Their policy status will remain pending until they personally sign it.</p></>}<label className="registration-full"><span>AA entity attribution (optional)</span><input onChange={(e) => updateScholarship("attribution", e.target.value)} placeholder="Meeting, YPAA committee, district, area, or state" value={order.scholarship.attribution} /></label></div> : null}</section>
